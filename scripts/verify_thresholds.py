...
def check_all() -> None:
    ...
    if all_verified:
        ...
        print(
            "   See source URLs above. After verifying, update verification_status to 'verified'."
        )
    print()
...
def show_snap_fy2026_summary() -> None:
    ...
    for size in ["1", "2", "3", "4", "5", "6", "7", "8"]:
        row = hh[size]
        print(
            f"{size:<10} ${row['gross_monthly']:>8,.0f}/month      ${row['net_monthly']:>8,.0f}/month   ${row['max_benefit']:>6,.0f}"
        )
    ea = hh["each_additional"]
    print(
        f"{'Each add.':<10} +${ea['gross_monthly']:>7,.0f}/month      +${ea['net_monthly']:>7,.0f}/month  +${ea['max_benefit']:>5,.0f}"
    )
    print(
