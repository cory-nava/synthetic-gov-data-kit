...
class TestEffectiveGrossLimit:
    ...
    def test_federal_gross_limit_matches_snap_table(self, va):
...
class TestEligibilityAbove130:
    def test_eligible_between_130_and_state_limit(self, va):
        # 3-person, gross $3,000: above federal $2,888 but below VA BBCE $4,442;
        # net below $2,221 → eligible under BBCE.
        eligible, reason = va.is_eligible(
            household_size=3, gross_income=3000, net_income=2000, liquid_assets=10_000
        )
        assert eligible
        assert "BBCE" in reason

    def test_ineligible_above_state_limit(self, va):
    ...
    def test_federally_ineligible_household_is_bbce_eligible(self, va):
        ...
        eligible, _ = va.is_eligible(
            household_size=3, gross_income=gross, net_income=2000, liquid_assets=0
        )
        assert eligible


class TestNetIncomeTestStillBinds:
    def test_net_test_fails_even_when_gross_passes(self, va):
...
class TestAssetRules:
    def test_waived_assets_ignored(self, va):
        eligible, _ = va.is_eligible(
            household_size=3, gross_income=3000, net_income=2000, liquid_assets=999_999
        )
        assert eligible

    def test_cap_state_fails_over_limit(self, tx):
        eligible, reason = tx.is_eligible(
            household_size=3, gross_income=3000, net_income=2000, liquid_assets=6000
        )
        assert not eligible
        assert "asset" in reason.lower()

    def test_cap_state_passes_under_limit(self, tx):
        eligible, _ = tx.is_eligible(
            household_size=3, gross_income=3000, net_income=2000, liquid_assets=4000
        )
        assert eligible


class TestNonBBCEDefersToFederal:
    def test_non_bbce_uses_federal_gross_limit(self, tn):
    ...
    def test_non_bbce_applies_federal_asset_limit(self, tn):
        # TN strict $3,000 asset limit applies via the federal path.
        eligible, reason = tn.is_eligible(
            household_size=3, gross_income=1500, net_income=1000, liquid_assets=3001
        )
        assert not eligible
        assert "asset" in reason.lower()
...
class TestMinimumBenefitFloor:
    def test_small_cat_el_household_floored(self, va):
        # 1-person, very high net income → calc benefit $0, floored to $24 minimum.
        benefit = va.estimate_monthly_benefit(
            household_size=1, net_income=5000, is_categorically_eligible=True
        )
        assert benefit == 24

    def test_large_household_not_floored(self, va):
        # 5-person household gets no minimum floor; high net income → $0.
        benefit = va.estimate_monthly_benefit(
            household_size=5, net_income=10_000, is_categorically_eligible=True
        )
        assert benefit == 0.0

    def test_benefit_reduced_by_30pct_net(self, va):
        # 3-person max allotment $785 - 30% of $1,000 net = $485.
        benefit = va.estimate_monthly_benefit(
            household_size=3, net_income=1000, is_categorically_eligible=True
        )
...
class TestDerivationMethodIntegrity:
    ...
    def test_ceil_method(self, pct, size, annual):
