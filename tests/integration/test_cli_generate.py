...
def test_generate_to_directory_creates_yaml_files(tmp_path: Path) -> None:
...
def test_generate_unknown_preset_exits_two() -> None:
    result = runner.invoke(app, ["generate", "snap.nowhere"])
    assert result.exit_code == 2


def test_generate_to_stdout_jsonl(tmp_path: Path) -> None:
...
def test_generate_to_stdout_yaml(tmp_path: Path) -> None:
...
def test_generate_multiple_formats(tmp_path: Path) -> None:
    result = runner.invoke(
        app,
        [
            "generate",
...
def test_generate_json_status_flag(tmp_path: Path) -> None:
...
def test_generate_quiet_flag_suppresses_rich_output(tmp_path: Path) -> None:
