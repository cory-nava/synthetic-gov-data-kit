...
def _parse_field_mapping(pairs: list[str]) -> dict[str, str]:
    ...
    for pair in pairs:
        if "=" not in pair:
            raise typer.BadParameter(f"--field-mapping expects CATALA_FIELD=PROFILE_FIELD, got '{pair}'")
...
def generate_catala(
    ruleset: Annotated[Path, typer.Argument(help="Path to a .catala_en/.catala_fr ruleset file")],
    scope: Annotated[str, typer.Option("--scope", help="Catala scope to run")],
    program: Annotated[str, typer.Option("--program", help="Program identifier, e.g. 'liheap'")],
    output: Annotated[Path, typer.Option("--output", "-o", help="Output directory")],
    jurisdiction: Annotated[str, typer.Option("--jurisdiction", help="Jurisdiction prefix, e.g. 'us'")] = "us",
    ...
    field_mapping: Annotated[
        ...
        typer.Option(
            "--field-mapping",
            help="CATALA_FIELD=PROFILE_FIELD override for fields with mismatched names (repeatable)",
        ),
    ] = None,
    citation: Annotated[str, typer.Option("--citation", help="Source citation recorded on generated cases")] = "",
    citation_year: Annotated[
        int | None, typer.Option("--citation-year", help="Year for the citation (default: now)")
    ] = None,
    binary: Annotated[str | None, typer.Option("--binary", help="Path to the catala executable")] = None,
    state: Annotated[str, typer.Option("--state", help="State code recorded on generated scenarios")] = "VA",
